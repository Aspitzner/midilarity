...
signatures = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']  # Modify to make it circular
...
def insert(title, artist, csv_tempo, bpm, time_signature, key_signature, avg_pressure, stress, file_name):
    ...
    cur.execute(
        "insert into midi(title,artist,csv_tempo,bpm,time_signature,key_signature,avg_pressure,stress,file,attributes) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",
