...
class Song(object):
    ...
    def __init__(self, title, artist, csv_tempo, bpm, time_signature, key_signature, avg_pressure, stress, file, distance):
        self.title = title
        self.artist = artist
...
def insert(title,artist,csv_tempo,bpm,time_signature,key_signature,avg_pressure,stress,file_name):
    ...
    attributes = '({0},{1},{2},{3})'.format(str(bpm),eval(time_signature)*time_multiplier,signature_index*key_multiplier,str(stress*stress_multiplier))
    
    cur.execute("insert into midi(title,artist,csv_tempo,bpm,time_signature,key_signature,avg_pressure,stress,file,attributes) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",(title,artist,str(csv_tempo),str(bpm),time_signature,key_signature,str(avg_pressure),str(stress),mybytearray,attributes))
...
def get_song(title,artist):
    ...
    cur.execute("SELECT title,artist,csv_tempo,bpm,time_signature,key_signature,avg_pressure,stress,file from midi where title = \'{0}\' and artist = \'{1}\'".format(title,artist))
    if cur.rowcount == 0:
        return None
    row = cur.fetchone()
    close_connection(con)
...
def get_intensity_tension_relationship():
    ...
    cur.execute("SELECT (SELECT SUM(avg_pressure) FROM midi)::DOUBLE PRECISION / (SELECT sum(stress) FROM midi) as ratio")
    row = cur.fetchone()
